...
#define NAVBTNW 20
@interface SaleHouseVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
- (NSArray *)leftTitles{
  ...
  @[@"*房源区域",@"*小区名称"],
  @[@"*售价",@"楼栋号",@"单元号",@"门牌号",@"*称呼"],
  @[@"楼层",@"楼层属性",@"*地图坐标",@"上传图片",@"*发布方式",@"*是否隐藏手机号码"]];
    }
    return _leftTitles;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.section) {
        ...
        default:{
            if (indexPath.row==5) {
                TSFHideNumCell * cell=[tableView dequeueReusableCellWithIdentifier:HIDECELL forIndexPath:indexPath];
                cell.label2.text=_model.hidetel;
                return cell;
            } else{
        ...
        case 2:{
            ...
            switch (indexPath.row) {
                ...
                case 5:{//是否隐藏手机号码
                    ...
                    __weak typeof(self)weakSelf=self;
                    if (_usermodel.ctel==nil || _usermodel.ctel.length==0) {//没有分机号
                                NSArray * array=@[@"公开",@"保密(请至个人中心申请分机号)"];
                                TSFOtherPickView * pick=[[TSFOtherPickView alloc]initWithFrame:weakSelf.view.bounds selectTitle:@"" array:array];
                                [pick showView:^(NSString *str) {
...
- (void)issueAction:(UIButton *)issueBtn{//发布
    ...
    if ( _model.jingweidu.length==0) {
        ...
        return;
    }  if ( _model.hidetel.length==0) {
        [YJProgressHUD showMessage:@"请选择是否隐藏手机号" inView:self.view];
        return;
    }
    ...
    if (_type==IssueTypeEdit) {//编辑
      
        [YJProgressHUD showProgress:@"正在加载中" inView:self.view];
        [ZYWHttpEngine AllPostURL:[NSString stringWithFormat:@"%@g=api&m=user&a=edit_ershou",URLSTR] params:param success:^(id responseObj) {
            [YJProgressHUD hide];
            if (responseObj) {
                ...
                if ([infomodel.success isEqual:@144]) {//编辑成功
    ...
    } else if(_type==IssueTypeGeneral){//发布
        [YJProgressHUD showProgress:@"正在加载中" inView:self.view];
