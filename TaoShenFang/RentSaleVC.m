...
@interface RentSaleVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title=@"发布出租房";
        ...
        case 2:{
            ...
            switch (indexPath.row) {
                ...
                case 2:{
                    ...
                    break;
                case 3:{
                    NSArray * array=@[@"直接出租",@"委托给经纪人",@"委托给平台"];
                    ...
                    [pick showView:^(NSString *str) {
                        if ([str isEqualToString:@"直接出租"]) {
                            weakSelf.model.pub_type=@"1";
...
- (void)issueAction:(UIButton *)button{
    ...
    if ( _model.hidetel.length==0) {
        [YJProgressHUD showMessage:@"请选择是否隐藏手机号码" inView:self.view];
        return;
    ...
    __weak typeof(self)weakSelf=self;
    if (_type==RentIssueTypeEdit) {//编辑
    
        [ZYWHttpEngine AllPostURL:[NSString stringWithFormat:@"%@g=api&m=user&a=edit_chuzu",URLSTR] params:param success:^(id responseObj) {
            if (responseObj) {
                ...
                if ([infomodel.success isEqual:@161]) {
