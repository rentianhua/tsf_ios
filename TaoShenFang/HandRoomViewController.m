...
@interface HandRoomViewController ()<UITableViewDelegate,UITableViewDataSource>{
    NSInteger _page;
    
}
@property (nonatomic,strong)NSMutableArray * latlonArray;//经纬度
...
@implementation HandRoomViewController

- (UIImageView *)BGView{
    if (_BGView==nil) {
        _BGView=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, kMainScreenWidth, kMainScreenHeight-64)];
        _BGView.image=[UIImage imageNamed:@"offline_bg_01"];
        _BGView.userInteractionEnabled=YES;
        UITapGestureRecognizer *recognizer=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapReloadData:)];
...
- (NSArray *)secArray{
    if (_secArray==nil) {
        _secArray=@[@"朝向",@"面积",@"标签",@"楼层",@"物业",@"来源",@"类型"];
    }
    return _secArray;
}
- (NSArray *)priceArray{
    if (_priceArray==nil) {
        _priceArray=@[@"不限",@"100万以下",@"100-200万",@"200-300万",@"300-400万",@"400万以上"];
    }
    return _priceArray;
}
...
- (NSMutableArray *)latlonArray{
    if (_latlonArray==nil) {
        _latlonArray=[NSMutableArray array];
    }
    return _latlonArray;
}
- (UIButton *)searchBtn{
   ...
   __weak typeof(self)weakSelf=self;
                ...
                if(moreStr.length>0)
                {
                    switch ([sec integerValue])
                    {
                        ...
                        case 2:{
                            ...
                            } else if ([moreStr isEqualToString:@"唯一住房"]){
                                
                                weakSelf.searchModel.wy=@"是";
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            if ([moreStr isEqualToString:@"业主"]) {
                                moreStr=@"yz";
                            } else if ([moreStr isEqualToString:@"经纪人"]){
                                moreStr=@"jjr";
                            }
                            weakSelf.searchModel.ly=moreStr;
                            break;
                        case 6:
    ...
    weakSelf.topView.btnBlock=^(TSFAreaModel * model,NSInteger areaindex,NSString * ditie,NSString * price,NSInteger priceindex,NSString * huxing,NSInteger huxingindex,NSString * moreStr,NSInteger section){
        ...
        if (priceindex==100) {
            ...
            if ([price isEqualToString:@"100万以下"]) {
                weakSelf.searchModel.zj=@"0-100";
            } else if ([price isEqualToString:@"400万以上"]){
                weakSelf.searchModel.zj=@"400-";
            } else{
...
- (void)loadData{
    ...
    [YJProgressHUD showProgress:@"正在加载中" inView:self.view];//====不管成功还是失败  都要隐藏
    self.searchModel.catid=@6;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.jianzhumianji==nil || [model.jianzhumianji isEqualToString:@"0"] ) {
        cell.label5.text=@"价格待定";
    } else{
        float mianji=[model.jianzhumianji floatValue];
        float zongjianum=[model.zongjia floatValue];
        float danjia=(zongjianum/mianji)*10000;
        cell.label5.text=[NSString stringWithFormat:@"%.f元/㎡",danjia];
    }
    return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HouseModel * handModel=self.dataArray[indexPath.row];
//    IDModel * model=[[IDModel alloc]init];
//    model.catid=handModel.catid;
//    model.ID=handModel.ID;
//    model.jjr_id=handModel.jjr_id;
//    model.username=handModel.username;
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if (self.latlonArray.count!=0) {
        [self.latlonArray removeAllObjects];
    }
    for (HouseModel * model in self.dataArray) {
        LatAndLonModel * latlonModel=[[LatAndLonModel alloc]init];
        latlonModel.latAndLon=model.jingweidu;
        latlonModel.xiaoqu=model.xiaoqu;
        [self.latlonArray addObject:latlonModel];
    }
}

- (void)tapReloadData:(UITapGestureRecognizer *)recognizer{
